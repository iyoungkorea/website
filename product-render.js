  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    window.productList.forEach(group => {
      ...
      const categoryId = 'cat-' + group.category.replace(/\s+/g, '-').replace(/[^\w\s-]/g, '').toLowerCase();
      section.innerHTML = `<h3 id="${categoryId}" class="gt4">${group.category}</h3>`;  
  
      // Special logic for Monitors
      if (group.category === "Monitor") {
        // Get all unique subCategories
        const subCategories = [...new Set(group.items.map(item => item.subCategory || "Other"))];
  
        subCategories.forEach(sub => {
          const subHeader = document.createElement("h4");
          subHeader.className = "monitor-subcategory";
          subHeader.textContent = sub;
          section.appendChild(subHeader);
  
          const subGroupContainer = document.createElement("div");
          subGroupContainer.className = "product-container";
  
          group.items
            .filter(item => (item.subCategory || "Other") === sub)
            .forEach(product => {
              ...
              `;
              subGroupContainer.appendChild(card);
            });
          section.appendChild(subGroupContainer);
        });
      } else {
        // Non-monitor categories: original render
